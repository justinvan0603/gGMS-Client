...
import { PluginService } from "./prdplugins.service";

@Component({
    // moduleId: module.id,

    selector: 'prdplugins',
    templateUrl: 'prdplugins-list.component.html',
...
export class PluginListComponent extends Paginated implements AfterViewChecked {
  ...
  validationMessages = {
    ...
    constructor(
        private dataService: PluginService,
    ...
    loadPlugins(searchString?:string) {
        ...
        this.dataService.getPlugins(this._page,searchString)
            .subscribe(res => {
...
ngAfterViewChecked(): void {
    ...
    priceChange()
    {
        if(this.selectedPlugin.Price != null)
        {
            this.selectedPlugin.PriceVat = (this.selectedPlugin.Price * this.selectedPlugin.Vat)/100;
        }
    }
    vatChange()
    {
        if(this.selectedPlugin.Vat != null)
        {
            this.selectedPlugin.PriceVat = (this.selectedPlugin.Price * this.selectedPlugin.Vat)/100;
        }
    }
    addNewPlugin(Plugin: PrdPlugin) {
        ...
        this.dataService.createPlugin(Plugin)
            .subscribe(rs=> {
                if(rs.Succeeded)
                {
   ...
   //     this.itemsService.addItemToStart<IScheduleT>(this.schedules, schedule);
    ...
    viewAddPlugin() {
        ...
        this.onEdit = false;
        this.addPlugin = new PrdPlugin();
        this.selectedPlugin = new PrdPlugin();
        this.addingPlugin = true;
        this.loadingBarService.complete();
        this.selectedPluginLoaded = true;
        this.childModal.show();

    }
deletePlugin(plugin:PrdPlugin)
{
    //console.log(domain);
    this.notificationService.openConfirmationDialog('Bạn có chắc muốn xóa?',
            () => {
                this.loadingBarService.start();
                this.dataService.deletePlugin(plugin.PluginId)
                    .subscribe(rs => {
                        if(rs.Succeeded)
                        {
                            this.notificationService.printSuccessMessage(rs.Message);
                            this.itemsService.removeItemFromArray<PrdPlugin>(this.Plugins, plugin);
                        }
                        else
                        {
                            this.notificationService.printErrorMessage(rs.Message);
...
editPlugin(plugin: PrdPlugin) {
        ...
        this.loadingBarService.start();
        this.onEdit = true;
        this.dataService.updatePlugin(this.selectedPlugin)
            .subscribe(res => {
                if(res.Succeeded)
                {
    ...
    public viewDetail(plugin: PrdPlugin)
    {
    ...
    public viewPluginDetails(plugin: PrdPlugin): void {
        this.isViewDetail = false;
        this.addingPlugin = false;
